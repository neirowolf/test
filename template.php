...
?>
<?$db_props = CIBlockElement::GetProperty(2, 27, array("sort" => "asc"), Array("CODE"=>"VOTES"));
...
<?foreach($arResult["ITEMS"] as $arItem):?>
	...
	<div class="bx-newslist-block">
		...
		<?foreach($arItem["DISPLAY_PROPERTIES"] as $pid=>$arProperty):?>
			...
			<?elseif ($arProperty["ID"] == "12"):?>
				<div class="bx-newslist-other"> 
					<?='Голосов: '. $value;?>
				</div>
				<div class="vote-btns">
					<input type="hidden" class="vote-value" value="<?=$value;?>">
					<input type="hidden" class="element-id" value="<?=$arItem["ID"];?>">
